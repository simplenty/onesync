...
impl TransferList {
    ...
    pub fn upsert(&self, file: SyncFile) {
        ...
        self.rows.borrow_mut().insert(key, transfer_row);
        self.reorder();
    }

    fn reorder(&self) {
        let mut rows: Vec<TransferRow> = self.rows.borrow().values().cloned().collect();
        rows.sort_by_key(|row| (row.completed.get(), Reverse(row.started_order.get())));
